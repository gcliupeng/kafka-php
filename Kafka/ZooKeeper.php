...
class ZooKeeper implements \Kafka\ClusterMetaData
{
    ...
    // }}}
    // {{{ public function listBrokers()
    public function initZk() {
        if ($this->zookeeper) {
            unset($this->zookeeper);
        }
        if (!is_null($this->timeout) && is_numeric($this->timeout)) {
            $this->zookeeper = new \ZooKeeper($this->hostList, null, $this->timeout);
        } else {
            $this->zookeeper = new \ZooKeeper($this->hostList);
        }
    }
