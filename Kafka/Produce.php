...
class Produce
{
     ...
     * @return void
     */
    public function send($reload = false)
    {
        $data = $this->_formatPayload($reload);
        if (empty($data)) {
            return false;
        }
     ...
     * @return array
     */
    public function getAvailablePartitions($topicName, $reload = false)
    {
        $topicDetail = $this->client->getTopicDetail($topicName, $reload);
     ...
     * @return array
     */
    private function _formatPayload($reload = false)
    {
        ...
        foreach ($this->payload as $topicName => $partitions) {
            foreach ($partitions as $partitionId => $messages) {
                $host = $this->client->getHostByPartition($topicName, $partitionId, $reload);
